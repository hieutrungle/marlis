...
from marlis.drl.agents import tqc
...
def main(config: TrainConfig):
    ...
    # Actor-Critic setup
    actor = tqc.Actor(envs, device=config.device)
    actor_detach = tqc.Actor(envs, device=config.device)
    if checkpoint != None:
    ...
    def get_q_params():
        qf1 = tqc.TQC(envs, n_quantiles=config.n_quantiles, device=config.device)
        qf2 = tqc.TQC(envs, n_quantiles=config.n_quantiles, device=config.device)
        qf3 = tqc.TQC(envs, n_quantiles=config.n_quantiles, device=config.device)

        qnet_params = from_modules(qf1, qf2, qf3, as_module=True)
        ...
        qnet = tqc.TQC(envs, n_quantiles=config.n_quantiles, device="meta")
